...
impl t500 {
    ...
    pub fn f602(&self, p0: &t501, p1: &t501, p2: &t501, p3: u32, p4: u32, p5: f32) -> Result<t501> {
        ensure!(
            p0.s507 == (p3 * p4) as usize,
            "layer_norm: input size mismatch"
        );
        ...
        if self.s509 {
            ...
            let (vx, vy) = if p3 <= 65535 {
                (p3, 1)
            } else {
                (65535, p3.div_ceil(65535))
            };
            self.f543(
                SHADER_LN_NORM_FUSED,
                Some("ln_norm_fused"),
                &v0,
                &[p0, p1, p2],
                &v1,
    ...
    pub fn f603(&self, p0: &t501, p1: &t501, p2: u32, p3: u32, p4: f32) -> Result<t501> {
        ensure!(
            p0.s507 == (p2 * p3) as usize,
            "rms_norm: input size mismatch"
        );
        ...
        let v14 = self.f503(v13 as usize);
        {
            ...
            let v18 = self.s500.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    wgpu::BindGroupEntry {
                        binding: 0,
                        resource: v15.as_entire_binding(),
                    },
                    wgpu::BindGroupEntry {
                        binding: 1,
                        resource: p1.s505.as_entire_binding(),
                    ...
                    wgpu::BindGroupEntry {
                        binding: 6,
                        resource: v14.s505.as_entire_binding(),
                    },
                ],
            });
        ...
        let v25 = self.f503(p4 as usize);
        {
            ...
            let v29 = self.s500.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    wgpu::BindGroupEntry {
                        binding: 0,
                        resource: v26.as_entire_binding(),
                    },
                    wgpu::BindGroupEntry {
                        binding: 1,
                        resource: p1.s505.as_entire_binding(),
                    ...
                    wgpu::BindGroupEntry {
                        binding: 4,
                        resource: v24.s505.as_entire_binding(),
                    },
                    wgpu::BindGroupEntry {
                        binding: 5,
                        resource: v25.s505.as_entire_binding(),
                    },
                ],
            });
    ...
    pub fn f791(
        ...
        p5: f32,
            ...
            p0.s507 == (p3 * p4) as usize,
            "ln_bwd: grad_output size mismatch"
        );
        ensure!(
            p1.s507 == (p3 * p4) as usize,
            "ln_bwd: fwd_input size mismatch"
        );
    ...
    pub fn f792(
        ...
        p5: f32,
            ...
            p0.s507 == (p3 * p4) as usize,
            "rn_bwd: grad_output size mismatch"
        );
        ensure!(
            p1.s507 == (p3 * p4) as usize,
            "rn_bwd: fwd_input size mismatch"
        );
...
mod tests {
    ...
    fn f600_per_channel() {
        ...
        let v3 = dev()
            .f504(&dev().f600(&v0, &v1, &v2, 1, 2, 2, 2, 1e-5).unwrap())
            .unwrap();
    ...
    fn f601_dinput_vs_numeric() {
        ...
        let v3 = dev()
            .f504(
                &dev()
                    .f600(&v0, &v1, &v2, batch, channels, spatial, groups, eps)
                    .unwrap(),
            )
            .unwrap();
        let n = v3.len() as f32;
        ...
        let (v6, _, _) = dev()
            .f601(&v4, &v0, &v5, batch, channels, spatial, groups, eps)
            .unwrap();
        ...
        for i in 0..n_elem {
            ...
            let out_plus = dev()
                .f504(
                    &dev()
                        .f600(&vp, &v1, &v2, batch, channels, spatial, groups, eps)
                        .unwrap(),
                )
                .unwrap();
            ...
            let out_minus = dev()
                .f504(
                    &dev()
                        .f600(&vm, &v1, &v2, batch, channels, spatial, groups, eps)
                        .unwrap(),
                )
                .unwrap();
    ...
    fn f601_dgamma_dbeta_known() {
        ...
        let (v3, v4, v5) = dev()
            .f601(&v0, &v1, &v2, batch, channels, spatial, groups, eps)
            .unwrap();
        ...
        assert!(
            (cpu_db[0] - 2.0).abs() < 1e-4 && (cpu_db[1] - 2.0).abs() < 1e-4,
            "cpu_db: {cpu_db:?}"
        );
    }
    ...
    fn cpu_layer_norm(
        ...
        for r in 0..rows {
            ...
            let var: f32 = input[base..base + cols]
                .iter()
                .map(|&x| (x - mean) * (x - mean))
                .sum::<f32>()
                / cols as f32;
    ...
    fn cpu_rms_norm(input: &[f32], weight: &[f32], rows: usize, cols: usize, eps: f32) -> Vec<f32> {
        ...
        for r in 0..rows {
            let base = r * cols;
            let mean_sq: f32 =
                input[base..base + cols].iter().map(|&x| x * x).sum::<f32>() / cols as f32;
    ...
    fn f603_unit_weight() {
        ...
        let v3 = dev()
            .f504(
                &dev()
                    .f603(&dev().f502(&v0), &dev().f502(&v1), 1, 2, 1e-5)
                    .unwrap(),
            )
            .unwrap();
        f544(&v3, &v2, 1e-3);
    ...
    fn f603_with_weight() {
        ...
        let v3 = dev()
            .f504(
                &dev()
                    .f603(&dev().f502(&v0), &dev().f502(&v1), 2, 3, 1e-5)
                    .unwrap(),
            )
            .unwrap();
        f544(&v3, &v2, 1e-3);
    ...
    fn f603_known_analytical_values() {
        ...
        let v2 = dev()
            .f504(
                &dev()
                    .f603(&dev().f502(&v0), &dev().f502(&v1), 1, 2, 0.0)
                    .unwrap(),
            )
            .unwrap();
    ...
    fn f603_known_with_per_col_weight() {
        ...
        let v2 = dev()
            .f504(
                &dev()
                    .f603(&dev().f502(&v0), &dev().f502(&v1), 1, 4, 0.0)
                    .unwrap(),
            )
            .unwrap();
    ...
    fn f603_size_mismatch() {
        ...
        assert!(
            dev()
                .f603(&dev().f502(&v0), &dev().f502(&v1), 3, 4, 1e-5)
                .is_err()
        );
    }
    ...
    fn f603_fused_stride_cols512() {
        ...
        let v0 = 4usize;
        let v1 = 512usize;
        let inp: Vec<f32> = (0..v0 * v1)
            .map(|i| ((i as f32) * 0.007 - 1.0).cos())
            .collect();
    ...
    fn f791_numeric_grad_check() {
        ...
        let (d_input_cpu, d_gamma_cpu, d_beta_cpu) =
            cpu_ln_bwd(&grad_y, &input, &gamma, rows, cols, eps);
    ...
    fn cpu_rn_bwd(
        ...
        for r in 0..rows {
            let base = r * cols;
            let sum_sq: f32 =
                input[base..base + cols].iter().map(|&v| v * v).sum::<f32>() / cols as f32;
            let inv_rms = 1.0 / (sum_sq + eps).sqrt();
            let s: f32 = (0..cols)
                .map(|c| grad_y[base + c] * gamma[c] * input[base + c])
                .sum::<f32>()
                / cols as f32;
            for c in 0..cols {
                d_input[base + c] = inv_rms * gamma[c] * grad_y[base + c]
                    - input[base + c] * (s * inv_rms * inv_rms * inv_rms);
