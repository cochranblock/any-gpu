...
impl t548 {
    ...
    pub fn f783(dev: &t500, model: &t538, pager: &t539, cfg: t547) -> Result<Self> {
        ...
        let lm_head_w = upload_and_transpose(
            dev,
            pager,
            model,
            "lm_head.weight",
        ...
        let kv_caches = (0..cfg.num_layers)
            .map(|_| t534::f672(dev, cfg.max_seq as u32, cfg.num_kv_heads as u32, hd as u32))
            .collect();
    ...
    pub fn f784(&mut self, dev: &t500, token_ids: &[u32]) -> Result<t501> {
            ...
            token_ids.len() <= self.cfg.max_seq,
            "prefill: {} tokens exceeds max_seq {}",
            token_ids.len(),
            self.cfg.max_seq
        );
    ...
    pub fn f785(&mut self, dev: &t500, token_id: u32) -> Result<t501> {
        let start_pos = self.kv_caches[0].f675(); // current cache length = next absolute pos
            ...
            start_pos < self.cfg.max_seq as u32,
            "decode_one: KV cache is full ({} tokens)",
            start_pos
        );
    ...
    fn layer_forward_batch_decode(
        ...
        let normed = dev.f603(
            x,
            &self.layers[layer_idx].attn_norm_w,
            active_batch,
            hdim,
            self.cfg.norm_eps,
        )?;
        ...
        let attn_out = dev.f580(
            &attn,
            &self.layers[layer_idx].o_w,
            active_batch,
            hdim,
            nh * hd,
        )?;
        ...
        let normed2 = dev.f603(
            &x,
            &self.layers[layer_idx].ffn_norm_w,
            active_batch,
            hdim,
            self.cfg.norm_eps,
        )?;
        ...
        let ffn_out = dev.f580(
            &ffn,
            &self.layers[layer_idx].down_w,
            active_batch,
            hdim,
            self.cfg.ff_dim as u32,
        )?;
    ...
    pub fn f788b(&mut self, dev: &t500, slot: usize, token_ids: &[u32]) -> Result<t501> {
            ...
            slot < self.cfg.max_batch.max(1),
            "f788b: slot {} >= max_batch {}",
            slot,
    ...
    pub fn f789b(&mut self, dev: &t500, next_tokens: &[u32]) -> Result<t501> {
        let active_batch = next_tokens.len() as u32;
            ...
            active_batch <= self.cfg.max_batch.max(1) as u32,
            "f789b: active_batch {} > max_batch {}",
            active_batch,
            self.cfg.max_batch.max(1)
        );
        ...
        for layer_idx in 0..self.cfg.num_layers {
            x =
                self.layer_forward_batch_decode(dev, &x, layer_idx, active_batch, &start_pos_buf)?;
        }
